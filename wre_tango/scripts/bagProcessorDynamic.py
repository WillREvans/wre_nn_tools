...
results_dir = directory + "dynamic_thresholded" + "_results" ###change this line as needed
...
def get_closest_vel(time):
	for topic, msg, t in bag.read_messages(topics=['/cmd_vel']):
		if (t < time):
			...
			return round(msg.linear.x, 5) #.angular.z , .linear.x
	return 0.0

with open(results_dir +"/"+filename+'_processed.csv', mode='w') as data_file:
	data_writer = csv.writer(data_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
	data_writer.writerow(['time', 'lidar_array', 'linear_velocity'])
